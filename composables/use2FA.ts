...
import { generateTwoFactorSecret, verifyTwoFactorToken, generateBackupCodes } from '~/utils/twoFactor/generator';
...
export const use2FA = () => {
  ...
  const generateSecret = async () => {
    ...
    try {
      ...
      const { secret: newSecret, qrCodeUrl: newQrCodeUrl } = await generateTwoFactorSecret(
      ...
      return { secret: newSecret, qrCodeUrl: newQrCodeUrl, backupCodes: backupCodes.value };
    } catch (error) {
  ...
  const verifyToken = (token: string, secretToVerify?: string): boolean => {
    ...
    return verifyTwoFactorToken(token, secretToUse);
  };
  ...
  const enable2FA = async (token: string): Promise<boolean> => {
    ...
    try {
      ...
      await updateDoc(doc($db, 'users', user.value.uid), { twoFactor: twoFactorData });
  ...
  const disable2FA = async (password: string): Promise<boolean> => {
    ...
    try {
      await updateDoc(doc($db, 'users', user.value.uid), { 'twoFactor.enabled': false });
  ...
  const regenerateBackupCodes = async (): Promise<string[]> => {
    ...
    try {
      ...
      await updateDoc(doc($db, 'users', user.value.uid), { 'twoFactor.backupCodes': newCodes });
